    ...
    public static void main(String[] args) {
        try {
            ...
            for (String cantonId : CANTON_NAMES.keySet()) {
                ...
                // Style uniforme : fond blanc + contour vert
            ...
            // ➕ Styles CSS dans le SVG
            Element style = doc.createElementNS("http://www.w3.org/2000/svg", "style");
            style.setTextContent(
                    ".canton { cursor: pointer; transition: all 0.2s; } " +
                            ".canton:hover { fill: #90EE90; stroke: darkgreen; stroke-width: 3px; } " +
                            ".selected { fill: #32CD32 !important; stroke: darkgreen !important; stroke-width: 3px !important; } " +
            ...
            script.setTextContent(
                    "function selectCanton(id) {" +
                            ...
                            "  window.location.href = '/annonces?canton=' + id;" +
                            "}"
            );
            doc.getDocumentElement().appendChild(script);

            // Ajouter labels au premier plan
            doc.getDocumentElement().appendChild(labelsGroup);

            // Sauvegarde
            try (OutputStreamWriter writer = new OutputStreamWriter(
                    new FileOutputStream("Suisse_cantons_styled.svg"), "UTF-8")) {
                org.apache.batik.dom.util.DOMUtilities.writeDocument(doc, writer);
            }

            System.out.println("🎉 Fichier généré : Suisse_cantons_styled.svg");
